...
class DialogueBowNetwork(nn.Module):
	...
	def forward(self, *input):
		[token_embeddings, input_mask_variable, conversation_mask, max_num_utterances_batch,
		 max_utterance_length , gold_next_bow, gold_prev_bow] = input
		...
		next_loss = self.masked_softmax(next_vocabulary_scores, gold_next_bow, input_mask_variable)
		prev_loss = self.masked_softmax(prev_vocab_scores, gold_prev_bow, input_mask_variable)
...
class DialogueClassifier(AbstractModel):
	...
	def add_args(parser):
		...
		network_parameters.add_argument("--T", type=int, help="Threshold to choose words from vocabulary", default=0.9)
