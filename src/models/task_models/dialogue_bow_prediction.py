...
class DialogueBowNetwork1(nn.Module):
	def __init__(self, args):
		super(DialogueBowNetwork1, self).__init__()
	...
	def forward(self, *input):
		...
		conversation_encoded, utterance_encoded = self.dialogue_embedder([token_embeddings, input_mask_variable, conversation_mask,
													   max_num_utterances_batch])
		conversation_batch_size = int(token_embeddings.shape[0] / max_num_utterances_batch)
		conversation_encoded = conversation_encoded.view(conversation_encoded.shape[0], 1, -1)
		## Get BOW Score
...
class DialogueClassifier(AbstractModel):
	...
	def evaluate_metrics(self, predicted, target, mask, mode = "dev"):
		...
		for i in range(next_predicted.shape[0]):
			...
			gold_ids = prev_correct[i][np.where(prev_correct[i] != 0)]
...
class DialogueBowNetwork(nn.Module):
	...
	def forward(self, *input):
		[token_embeddings, input_mask_variable, conversation_mask, max_num_utterances_batch,
		gold_bow, gold_mask, gold_weights] = input
		...
		bow_prev_weights = bow_weights_reassembled[:, 0:-1].contiguous()
		...
		bow_next_weights = bow_next_weights.view(bow_next_weights.shape[0]*bow_next_weights.shape[1], -1)
		bow_prev_weights = bow_prev_weights.view(bow_prev_weights.shape[0] * bow_prev_weights.shape[1], -1)
		...
		# next_vocab_scores = self.next_bow_scorer(utterance_encodings_next)
		# prev_vocab_scores = self.prev_bow_scorer(utterance_encodings_prev)

		## Computing custom masked cross entropy
		next_loss = self.multilabel_cross_entropy(next_vocab_scores, bow_next, conversation_mask_next, bow_next_weights)
	...
	def evaluate(self, *input):
		...
		next_vocab_probabilities = F.sigmoid(next_vocab_scores)
		prev_vocab_probabilities = F.sigmoid(prev_vocab_scores)
...
class DialogueClassifier(AbstractModel):
	...
	def target(self, inputs):
		...
		# Convert to CPU
		if self.args.use_cuda:
			utterance_bow = inputs[-3].data.cpu()
			input_mask = inputs[2].data.cpu()
		else:
			utterance_bow = inputs[-3].data
	...
	def evaluate_metrics(self, predicted, target, mask, mode = "dev"):
		...
		prev_mask = mask[:, 0:-1].contiguous().view(-1, 1).long()
		next_predicted = predicted[0].numpy()
		prev_predicted = predicted[1].numpy()
		...
		prev_correct = target_reassembled[:, 0:-1, :].contiguous().view(prev_mask.shape[0],-1)
		if self.args.use_cuda:
		...
		for i in range(next_predicted.shape[0]):
			predicted_ids = np.where(next_predicted[i] > self.args.threshold)[0] # remove start, end, pad token
			predicted_words = self.vocabulary.get_tokens(predicted_ids)
			# if len(predicted_words) > 0:
			# 	logger.info(" ".join(predicted_words))
			gold_ids = np.where(next_correct[i] != 0)[0]
			if len(gold_ids) == 0:
				continue
		...
		metric_update_dict["precision"] = [batch_precision, total]
		metric_update_dict["recall"] = [batch_recall, total]
		metric_update_dict["f1"] = [batch_f1,  total]
		return metric_update_dict
	...
	def vectorize(self, batch, mode = "train"):
		...
		## For binary cross entropy, use vector multiplication over entire vocaulary
		# bow_list =

		if mode == "train":
			return batch_size, token_embeddings, input_mask_variable, conversation_mask, max_num_utterances_batch, \
				bow_list, bow_mask, bow_weights
		else:
			return batch_size, token_embeddings, input_mask_variable, conversation_mask, max_num_utterances_batch
